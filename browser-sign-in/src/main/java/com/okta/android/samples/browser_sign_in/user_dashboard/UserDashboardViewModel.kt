...
class UserDashboardViewModel @Inject constructor() : ViewModel() {
    ...
    fun refreshToken() {
        viewModelScope.launch {
           CredentialBootstrap.defaultCredential().refreshToken()

            val idToken = CredentialBootstrap.defaultCredential().idToken()
            val id_token = CredentialBootstrap.defaultCredential().token?.idToken
            val accessToken = CredentialBootstrap.defaultCredential().token?.accessToken
            val refreshToken = CredentialBootstrap.defaultCredential().token?.refreshToken
            if (id_token == null) {
                ...
                System.out.println("accesss is:"+accesstoken)
